...
// Reuse the same styles as SentinelExplorer
...
const MISSIONS = [
  { id: 'all',        label: 'All' },
  { id: 'landsat-9',  label: 'L9' },
  { id: 'landsat-8',  label: 'L8' },
  { id: 'landsat-7',  label: 'L7' },
  { id: 'landsat-5',  label: 'L5' },
  { id: 'landsat-4',  label: 'L4' },
];

const BAND_OPTIONS = [
  { value: 'true-color',  label: 'True Color (RGB)',        icon: '🌍' },
  { value: 'false-color', label: 'False Color (NIR-R-G)',   icon: '🌿' },
...
const LandsatExplorer = () => {
  const store = useLandsatExplorerStore();
  ...
  const handleAddToMap = useCallback((result) => {
    ...
    const layerId = `landsat_${result.id}_${bands}_${Date.now()}`;

    // Build XYZ tile URL from Planetary Computer
    const tileUrl = buildTileUrl(result.id, bands);
    ...
    <div className="fire-controls">
      <div className="fire-controls__header">
        <div className="fire-controls__toggle" onClick={() => setIsExpanded((v) => !v)}>
          ...
          <span className="fire-controls__toggle-label">Landsat Explorer</span>
        </div>
        <button
                              ...
                              <div className={styles.cardBody}>
                                ...
                                <div className={styles.cardMeta}>
                                  ...
                                  {result.collectionCategory && (
                                    <span className={styles.cardType}>Tier {result.collectionCategory}</span>
                                  )}
                                </div>
                              ...
                              <div className={styles.cardDetails}>
                                <div className={styles.detailGrid}>
                                  ...
                                  <div className={styles.detailItem}>
                                    ...
                                    <span className={styles.detailValue}>{result.instruments || 'N/A'}</span>
                                  </div>
                                  <div className={styles.detailItem}>
                                    <span className={styles.detailLabel}>WRS Path/Row</span>
                                    <span className={styles.detailValue}>{result.wrsPath}/{result.wrsRow}</span>
                                  </div>
                                  {result.sunElevation != null && (
                                    <div className={styles.detailItem}>
                                      <span className={styles.detailLabel}>Sun Elevation</span>
                                      <span className={styles.detailValue}>{result.sunElevation.toFixed(1)}°</span>
                                    </div>
                                  )}
                                  {result.cloudCoverLand != null && (
                                    <div className={styles.detailItem}>
                                      <span className={styles.detailLabel}>Cloud Cover (land)</span>
                                      <span className={styles.detailValue}>{result.cloudCoverLand.toFixed(1)}%</span>
                                    </div>
                                  )}
                                  <div className={styles.detailItem}>
                                    <span className={styles.detailLabel}>Tier</span>
                                    <span className={styles.detailValue}>{result.collectionCategory || 'N/A'}</span>
                                  </div>
                                </div>
                              </div>
                            )}
