...
const BAND_OPTIONS = {
  'sentinel-2': [
    { value: 'true-color',  label: 'Натуральные цвета (RGB)' },
    { value: 'false-color', label: 'Ложный цвет (БИК-К-З)'  },
    { value: 'ndvi',        label: 'NDVI (Растительность)'   },
    { value: 'ndwi',        label: 'NDWI (Водные объекты)'   },
    { value: 'ndbr',        label: 'NDBR (Индекс выжженности)' },
  ],
  'sentinel-3': [
  ...
  'sentinel-1': [
    { value: 'IW_VV', label: 'IW VV' },
  ],
};

// TODO: change sentinel numbering
...
const SentinelExplorerOld = () => {
  const store = useSentinelExplorerOldStore();
    ...
    <div className="fire-controls">
      <div className="fire-controls__header">
        <div className="fire-controls__toggle" onClick={() => setIsExpanded((v) => !v)}>
          ...
          <span className="fire-controls__toggle-label">Sentinel Explorer (Legacy CDSE)</span>
        </div>
        <button
          ...
          <div className={styles.content}>
              ...
              <div className={styles.searchSection}>
                ...
                <div className={styles.dateRow}>
                  <div className={styles.field}>
                    <label className={styles.label}><Calendar size={12} /> Начало</label>
                    <input
                      type="date"
                  ...
                  <div className={styles.field}>
                    <label className={styles.label}><Calendar size={12} /> Конец</label>
                    <input
                      type="date"
                      value={store.endDate}
                ...
                <div className={styles.field}>
                  <label className={styles.label}>Непрозрачность: {store.globalOpacity}%</label>
                  <input
                    type="range" min="0" max="100"
              ...
              <div className={styles.resultsSection}>
                {store.isLoading && (
                  <div className={styles.loadingMsg}>
                    <div className={styles.spinner} /><div>Поиск снимков…</div>
                  </div>
                )}
                            ...
                            <div className={styles.cardMain}>
                              <div className={styles.cardThumb}>
                                {result.thumbnailUrl && !thumbFailed ? (
                                  <img src={result.thumbnailUrl} alt="preview" loading="lazy"
                              ...
                              <div className={styles.cardBody}>
                                <div className={styles.cardTopRow}>
                                  ...
                                  <span className={styles.missionBadge} style={{ background: getMissionColor(result.mission) }}>
                                    {getMissionLabel(result.mission)}
                                  </span>
                                </div>
                                <div className={styles.cardMeta}>
                                  {result.cloudCover != null && (
                                    <span className={styles.cloudBadge} style={{ color: getCloudCoverColor(result.cloudCover) }}>
                                      <Cloud size={11} /> {result.cloudCover.toFixed(1)}%
                                      <span className={styles.cardType}>{getCloudCoverLabel(result.cloudCover)}</span>
                                    </span>
                                  )}
                            ...
                            {isDetailExpanded && (
                              <div className={styles.cardDetails}>
                                <div className={styles.detailGrid}>
                                  {[
                                    ['ID сцены',          result.sceneId],
                                    ['Платформа',         result.platform],
                                    ['Инструмент',        result.instrument],
                                    ['Уровень обработки', result.processingLevel],
                                    result.orbitDirection ? ['Направление орбиты', result.orbitDirection] : null,
                                    result.orbitNumber    ? ['Номер орбиты',       result.orbitNumber]    : null,
                                    result.size           ? ['Размер',             formatFileSize(result.size)] : null,
            ...
            {store.activeTab === 'layers' && (
              <div className={styles.layersSection}>
                {store.activeLayers.length === 0 ? (
                  <div className={styles.emptyLayers}><Layers size={28} /><div>Нет добавленных слоёв</div></div>
                ) : (
                  <>
                    <div className={styles.layersHeader}>
                      <span className={styles.totalCount}>{store.activeLayers.length} слоёв на карте</span>
                          ...
                          <div className={styles.layerTop}>
                            <div className={styles.layerInfo}>
                              <div className={styles.layerName}>
                                <span className={styles.missionBadge} style={{
                                  background: getMissionColor(layer.mission),
                                  marginRight: '0.35rem', fontSize: '0.55rem', padding: '0.05rem 0.3rem',
                                }}>
                            ...
                            <div className={styles.layerActions}>
                              {idx > 0 && (
                                <button className={`${styles.layerActionBtn} ${styles['layerActionBtn--up']}`}
                                  onClick={() => handleReorder(idx, idx - 1)}><ArrowUp size={12} /></button>
                              )}
                              {idx < store.activeLayers.length - 1 && (
                                <button className={`${styles.layerActionBtn} ${styles['layerActionBtn--down']}`}
                                  onClick={() => handleReorder(idx, idx + 1)}><ArrowDown size={12} /></button>
                              )}
                              <button
                              ...
                              <button className={`${styles.layerActionBtn} ${styles['layerActionBtn--danger']}`}
                                onClick={() => handleRemoveLayer(layer.id)}><Trash2 size={12} /></button>
                            </div>
                          </div>
...
export default SentinelExplorerOld;
